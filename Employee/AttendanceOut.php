...
if (array_key_exists('email', $_SESSION) && !empty($_SESSION['email'])) {
    ...
    if ( !is_null($rowq["branchid2"]) || !is_null($rowq["branchid3"]) || !is_null($rowq["branchid4"]) || !is_null($rowq["branchid5"])){
        header('Location: AttendanceExecOut.php');
    }
...
<html>
    <head>
        ...
        while ($row = mysqli_fetch_array($result)) {
            if(!is_null($row) && $row["timeout"] == "00:00:00" ){
                    ...
                    <div class="col-md-3"> <!-- left panel -->
                        <div class="panel panel-default" style="box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.1), 0 3px 10px 0 rgba(0, 0, 0, 0.1);">
                            <div class="panel-body" style="margin-left: 10px;margin-right: 10px;">
                                ...
                                <br/>
                                <a href="AttendanceComplete.php"><button type="button" class="btn btn-primary btn-sm"><i class="fa fa-area-chart"></i> This Month Attendance Summary</button></a>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="panel panel-default" style="box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.1), 0 3px 10px 0 rgba(0, 0, 0, 0.1);">
                            <div class="panel-body" style="margin-left: 10px;margin-right: 10px;">
                                <div class="text-center">
                                    <form action="validationOut.php" method="post">
                                        <h4 class="text-success">Tracker Details</h4>
                                        <input type="hidden" value="<?php echo $_SESSION["email"]; ?>" name="email" id="email"/>
                                        <label>Date </label><input text-align="center" type="text" name="attendancedate" id="attendancedate" value="<?php echo $currentdate; ?>" class="form-control" readonly/><br/>
