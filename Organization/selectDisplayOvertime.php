...
if (isset($_POST["email"])) {
    ...
    $query = "select *,TIME_FORMAT(timeinot,'%H:%i') as timeinsim, TIME_FORMAT(timeoutot,'%H:%i') as timeoutsim from overtimerecord where email = '$email' and attendancedateot >= '$start' and attendancedateot <= '$end'";
    $query2 = "select sum(totalhoursot) as totalhours,sum(totalminsot) as totalmins from overtimerecord where email='$email' and attendancedateot >= '$start' and attendancedateot <= '$end'";
    $result2 = mysqli_query($con, $query2);

    $output = '
    <div>
            Overtime Record from <span style="font-size:20px;">' . $start . '</span> to <span style="font-size:20px;">' . $end . '</span>.<br/>';
    ...
    $output .= 'Total Working Hours : <span style="font-size:20px;">' . $totalhours . '</span> Hours <span style="font-size:20px;">' . $totalmins . '</span> Minutes.<br/>
                ...
                <td><label>Reason for Overtime</label></td>
    ...
    while ($row = mysqli_fetch_array($result)) {
        $output .= '  
                <tr>
                <td>' . $row["attendancedateot"] . '</td>
                <td>' . $row["timeinsim"] . '</td>
                <td>' . $row["timeoutsim"] . '</td>
                <td>' . $row["reasonot"] . '</td>
                <td>' . $row["totalhoursot"] . '</td>
                <td>' . $row["totalminsot"] . '</td>
                </tr>
                ';
    }
    }else {
